...
public class Ex47_final {
	...
	public static void main(String[] args) {
		...
		// - 상수명(fical 변수명)은 항상 모두 대문자로 만든다.(관습)
...
class FinalItem {
	...
	public final int C = 30;	// 멤버 상수는 접근 지정자를 public으로 하는 경우가 많다.(굳이 모든객체가 같응ㄴ 갑을 가짐)
	
	public final static int D = 40; //정적 상수 -> 가장 많이 사용(값을 고정할때 모든 값을 고정하니까/ 물리적으로 이득)
}


//홍길동 제작
// -> 길동이는 FinalParent 클래스가 부모가 되길 바라지 않음. 상속에 사용되지 않았으면 좋겠다.
// -> 더이상 자식을 가질 수 없는 클래스 > 최종 클래스
...
//class FinalChild extends FinalParent {//The type FinalChild cannot subclass the final class FinalParent
	...
	//고치지 말라는 메소드를 고쳤다!! -> 예기치못한 문제 발생 가능성 생김!!
	//@Override
