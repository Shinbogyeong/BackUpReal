	...
	private static void m9() {
		...
		name.sort(new Comparator<String>() {
			...
			public int compare(String o1, String o2) {
				...
				//return o1.length() - o2.length();// 글자수
...
//		});
	...
	private static void m5() {
		...
		for (int i=0; i<list.length-1; i++) {
			...
			for (int j=0; j<list.length-1-i; j++) {
				...
				//우위 비교
				// - 객체간의 비교가 아니라 -> 객체가 가지는 단일값의 비교(O)
	...
	private static void m2() {
		...
		int[] num = { 10, 30, 30, 50 ,30};
		System.out.println(num); //[I@3830f1c0
		System.out.println(num.toString());
		System.out.println(Arrays.toString(num)); //[10, 30, 30, 50, 30] ****
	...
	private static void m1() {
		/*
		 ...
		 a. 데이터 집합
		 		...
		 		2. HaxhMap: 데이터 집합 x 1개
		 
		 b. 행동 집합
		 		1. 실명 클래스 : 객체화 횟수 x N개 + 클래스 선언 비용O
		 		2. 익명 클래스 : 객체화 횟수 x N개 + 클래스 선언 비용X 
		...
		//인터페이스를 상속한 클래스의 객체는 괴고록 인터페이스 변수를 만들어서 담아라
		AAA c2  = new CCC();//업캐스팅
		c2.test();
		
		//Cannot instantiate the type AAA d 인터페이스는 스스로객체를 못만듬
		// 추상 메소드 때문에
		//AAA c3 = new AAA();
		...
		AAA c3 = new AAA() {
			...
			@Override
			public void test() {
				System.out.println("인터페이스를 구현한 메소드222");
			}
		};
...
interface AAA {// 얘는 무조건 추상메서드밖에 못옴
	
	//public abstract void test();//추상 메서드, private못옴 밖에서 보여야하니까ㅏ
	void test();//(public, abstract)생략가능
...
class CCC implements AAA{ //인터페이스를 상속받는 ㅋ클래스 선언
...
abstract class BBB { //얘는 추상메서드뿐만아니라 변수나 메서드 옴
