...
public class Ex48_Interface {
	...
	public static void main(String[] args) {
		...
		// - 클래스의 일종(자료형)
		// - 클래스의 구조도 상이, 목적도 상이
		// - 인터페이스는 객체를 생성할 수 없다. -> 추상 메소드 때문에.. -> 실체화된 객체가 구현부를 가지지 못한 메소드를 호출하기 없기 때문에...
		...
		//Phone p = new Phone();
		//p.test(); //이작업이 불가능하기 때문에 인터페이스는 객체를 생성할 수 없다.
...
interface Phone {
	...
	//{} 없음 -> 구현부가 없는 메소드 -> 실체화가 되지 않은 메소드 -> 추상메소드(Abstract Method) -> 호출해도 사용이 불가능 하다. -> 현 상태로는 사용할 수 없다.
	//public void test(); 
...
class S21 implements Phone{ //상속
	...
	//일반 상속 -> 오버라이드 -> 개발자의 선택
	//인터페이스 상속 -> 오버라이드 -> 개발자의 선택여지 없음 -> 무조건 필수!! -> 안하면 컴파일 오류 -> 강제!!
	public void test() {
...
//일반 클래스는 추상 메소드를 가질수 없다.(***)
class Test {
	//The abstract method test in type Test can only be defined by an abstract class
	//public abstract void test();
