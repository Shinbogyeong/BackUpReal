...
//		}
	...
	private static void m5() {
		...
		//실행(디버그 없이): Ctrl + F11
		//실행(디버그 모드): F11 (breakpoint가 동작함, 그줄 실행 전까지 실행)
		// - F5(라인을 하나씩 실행한다.
		// - Variables 창(현재 상태의 변수와 값을 실시간으로 보여준다.)
		// - F8(나머지 코드를 한번에 실행한다.)
		// - 오른쪽 디버깅모드말구 자바로 돌아온다
		
		//Ctrl + Shift + b = 브레이크 포인트
		
		
//		int sum; //null상태(연산이나 행동의 대상이 될 수 없다.) -> 0과 다른 의미
		int sum = 0;
	...
	private static void m4() {
		...
		//무언가의 누적값을 구하는 방법 
		// 1. 누적 변수를 선언한다. 보통 누적 변수는 0으로 초기화한다.
		int sum2 = 0;
	...
	private static void m2() {
		...
		//for문 목적
		//. 반복 횟수 제어 -> 루프 변수 값을 사용해서.. -> 쉬웅
		//2. 루프 변수 값 제어 -> 쉬움 ~ 어려움(****)
		
		
		// 지역 변수
		// - 매소드내에서 선언한 변수 > 생명주기(영역) > 메소드를 영역으로..
...
//		
		...
		//최종 & 우리 결론 (선택)
		// - 루프 변수의 역할이란? 단순히 회전수를 제어하는 것뿐만 아니라.. 요구에 필요한 값을 제어(제공)하는데 사용된다.(중요!! 어려움)
		for (int i=1; i<=10; i++) {
