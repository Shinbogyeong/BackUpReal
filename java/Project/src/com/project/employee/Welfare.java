...
/**
 * 복지 포인트의 일을 수행하는 클래스이다.
 * 복지 포인트는 사원들마다 다르고, 정해진 것 이상으로는 구매할수 없다.
 * 또한 복지포인트는 당연히 로그인된 사용자마다 다르다.
 * @author 3조
 */
...
//		list = new ArrayList<getpoint>();
    ...
    * 1. 잔여 포인트, 2번 상품목록, 3번 사용내역, 4번 종료하기
    * @return
    */
   ...
   /**
    * 복지포인트의 메뉴를 메서드로 만들어준 코드이다.
    * 날짜와 시간을 받아주고 메뉴에서 1,2,3,4를 누르면
    * 그에 해당하는 메서드가 호출된다.
    ...
    * 
    * @param user employee를 user로 선언해준뒤에, 
    */
    ...
    * 
    * @param now_date buy_product에 있는 값을 매개변수로 받기 위해서 사용했다.
    * @param employee employee에 있는 값들을 매개변수로 받기 위해 사용 했다. 
    * @throws FileNotFoundException
    */
    ...
    * 1. 전자제품, 2.가전제품, 3.여행패키지, 4.건강패키지
    * @return proname 선택한 번호를 return 해준다.
    */
   ...
   /**
    * 처음 어떤 종류의 상품을 구매하는지에 따라서 1,2,3,4 중에 선택한다.
    * 그 선택한 번호는 데이터가 그 번호의 더미파일로 저장된다.
    * product_list는 choosed로 사용하고, 상품목록에 관한 더미파일들을 읽어준다. 
    * 그리고 product_list라는 arraylist에 product 번호, 이름, 가격을 저장해준다.
    * @param choosed product_List를 choosed로 선언해준다.
    * @return select 구매한 물건 번호를 담는다.
    * @throws FileNotFoundException
    */
   ...
   /**
    * 각 사용자마다 사용 가능한 복지 포인트를 보여주는 메소드이고, 
    * 로그인한 사람에 따라서 가지고 있는 포인트가 다르게 나온다.
    * 
    */
   ...
   /**
    * 
    * 이 값들을 배열에 0,1,2,3,4순으로 물건 번호, 이름, 가격, 사원번호, 날짜를 
    * buy_list라는 arraylist에 저장시켜준다.
    * @throws FileNotFoundException
    */
   ...
   /**
    * EMPLOYEE라는 파일경로에 있는 파일을 ■마다 자른뒤에 그 값을 load_employee에다가 넣어주고 
    * employee arraylist에 저장시켜준다.
    */
    ...
    * 
    * @param user employee값을 user 매개변수로 사용하기 위해서 사용했다.
    */
