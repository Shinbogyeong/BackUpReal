...
- 프로젝트 하실 때 DBMS 뭐 쓰셨어요? -> Oracle 11g 사용했습니다.
- 프로젝트 하실 때 DB 클라이언트는 뭐 쓰셨어요? -> SQL Developper 사용했습니다.
...
4. 데이터 중복 최소화
    - 동일한 데이터가 여러 곳에 여러번 저장되는 것을 방지한다.
...
선생님, 노트북, 세팅, 갑자기 오라클 안되요...? 오라클을 안깔았어!
...
열려 있는 파일
-> 스크립트 파일(*.sql), 워크시크 파일(*.sql)
-> 오라클(데이터베이스 서버)와 대화를 하기 윈한 작업 파일(자바 -> 소스파일(*.java)


스크립트 실행 방법
1. 실행할 명령어(문장)을 선택한다.(블럭)
    - 마우스
    - shift + 방향키(home.end)
    + ctrl + enter
...
--데이터베이스 서버 설치(오라클 서버) -> 데이터베이스 클라이언트 틀( SQL Developer) -> hr 계ㄷ정활성화 ->  DB접속
...
- 구조화된 질의 언어
- 사용자(클라이언트 롤)가 관계형 데이터베이스와 대화할 때 사용하는 언어
- 자바에 비해 자연어에 가깝다.
...
2. 표준  SQL(ANSI-SQL)
- 모든 DBMS는 표준 SQL를 지우너하도록 설계되어 있다.
...
--  현재계정(HR)이 소유하고 있는 테이블 목록 보여주세요.
