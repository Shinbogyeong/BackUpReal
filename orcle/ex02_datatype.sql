...
1. 숫자형
    ...
    a. number
        ...
        - number(precision, scale)
            ...
            ex) number -> 38자리까지 표현 가능하나 모든 숫자(정수, 실수 포함)
...
insert into tblType (num) values (1234567890123456789012345678901234567890); --38자리
...
2. 문자형
    ...
    a. char  (***********)
        ...
        - char(n) : n자리 문자열, n(바이트)
            ...
            - 최대 크기: 2000바이트
            ex) char  -> X
            ex) char(3)  -> 최대 3바이트까지 문자열을 저장 -> 인코딩? -> 영어(3글자), 한글(1글자) utf8
            ex) char(10) -> 최대 10바이트까지 문자열을 저장 -> 영어(10글자), 한글(3글자)
            ex) char(2000) -> 영어(2000글자), 한글(666글자)
    
    b. nchar
        - n : national > 유니코드 지원 > UTF-16으로 동작하겠습니다.(모든문자 -> 2바이트 취급)
...
inset into tblType (name, nick) values ('홍길동', '홍길동');
...
insert into tblType (name, age, birthday) values ('홍길동', 20, '1995-5-15');
