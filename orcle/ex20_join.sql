...
-- A.3 신규 프로젝트 추가(X)
-- ORA-02291: 무결성 제약조건(HR.SYS_C007639)이 위배되었습니다- 부모 키가 없습니다
insert into tblProject (seq, project, staff_seq) values (6, '고객유치', 5);


-- B. '홍길동' 퇴사
-- B.1 '홍길동' 삭제(X)
-- ORA-02292: 무결성 제약조건(HR.SYS_C007639)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
2. 자식 테이블 조작. tblProject
    ...
    b. 행수정(update) : 수정할 컬럼이 FK 컬럼 > 부모 테이블 존재하는 값인지 사전 체크 요망!!! > parent record not found
...
-- 1. 모든 테이블 데이터 삭제  -> delete from 테이블;
-- 2. 모든 sequence 객체 삭제 -> drop sequence 4개;
-- 3. 모든 sequence 객체 생성 -> create sequence 4개;
-- 4. insert문들 실행
...
3. 외부 조인, outer join (******)
    ...
    - 일반적으로 부모 테이블 가르킨다.(left or right)
...
select * from tblCustomer c left outer join tblSales s on c.seq = s.cseq;
...
create table tblSelf (
    ...
    seq number primary key,                             --직원번호(PK)
    name varchar2(30) not null,                         --직원명
    deparment varchar2(50) null,                        --부서명
